...
from apps.stores.models import StoreBookSubscription
...
def process_subscription_notification(store_book_id):
    subscribers = StoreBookSubscription.objects.select_related(
        'store_book', 'subscriber', 'store_book__book').filter(store_book_id=store_book_id).all()
    if subscribers:
        send_email(
...
def send_email(subject, message, sender, to_list, fail_silently, **kwargs):
    ...
    result = django_send_mail(subject, message, sender, to_list, fail_silently=fail_silently, **kwargs)
