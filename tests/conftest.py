...
from tests.stores.factories import StoreFactory, StoreBookFactory, StoreBookSubscriptionFactory
...
@pytest.fixture
def app_admin():
    user = UserFactory(is_superuser=True, is_staff=True, username='admin', email='admin@dmin.com')
    return user
...
def build_store_book_subscription_list():
    def _wrap(owner=None, batch_count=5):
        ...
        for store_book, user in zip(store_books, users):
            store_book_subscriptions.append(StoreBookSubscriptionFactory(store_book=store_book, subscriber=user))
